...
from ironic.drivers.modules import diskless_deploy
...
def _parse_deploy_info(node):
    ...
    """
    info = {}
    info.update(diskless_deploy.parse_instance_info(node))
...
def _build_pxe_config_options(node, pxe_info, ctx):
    ...
    deploy_ramdisk_options = diskless_deploy.build_deploy_ramdisk_options(node)
...
def validate_boot_option_for_uefi(node):
    ...
    boot_option = diskless_deploy.get_boot_option(node)
...
class PXEDeploy(base.DeployInterface):
    ...
    def validate(self, task):
        ...
        diskless_deploy.validate_image_properties(task.context, d_info, props)
    ...
    def deploy(self, task):
        ...
        """
        diskless_deploy.cache_instance_image(task.context, task.node)
        diskless_deploy.check_image_size(task)
        ...
        # TODO(blakec): Not implemented yet
    ...
    def clean_up(self, task):
        ...
        diskless_deploy.destroy_images(node.uuid)
        _destroy_token_file(node)

    def take_over(self, task):
        if not diskless_deploy.get_boot_option(task.node) == "local":
    ...
    def validate(self, task, method, **kwargs):
        ...
        * pass_deploy_info
        ...
        if method == 'pass_deploy_info':
            ...
            diskless_deploy.get_deploy_info(task.node, **kwargs)

    @base.passthru(['POST'])
    @task_manager.require_exclusive_lock
    def pass_deploy_info(self, task, **kwargs):
        """Continues the deployment of diskless baremetal node.

        This method continues the deployment of the baremetal node

        :param task: a TaskManager instance containing the node to act on.
        :param kwargs: kwargs for performing diskless deployment.
        :raises: InvalidState
        """
        node = task.node
        ...
        uuid_dict = diskless_deploy.continue_deploy(task, **kwargs)
        ...
        try:
            ...
            msg = _('Failed to continue diskless deployment.')
            deploy_utils.set_failed_state(task, msg)
        else:
            diskless_deploy.finish_deploy(task, kwargs.get('address'))
